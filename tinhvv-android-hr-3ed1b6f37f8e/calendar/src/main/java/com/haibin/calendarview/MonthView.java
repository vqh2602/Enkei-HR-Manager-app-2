...
public abstract class MonthView extends BaseMonthView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mLineCount; i++) {
            for (int j = 0; j < 7; j++) {
                ...
                if (mDelegate
                    .getMonthViewShowMode() == CalendarViewDelegate.MODE_ONLY_CURRENT_MONTH) {
                    ...
                    .getMonthViewShowMode() == CalendarViewDelegate.MODE_FIT_MONTH) {
                    if (d >= count) {
                        return;
                    }
                }
    ...
    private void draw(Canvas canvas, Calendar calendar, int i, int j, int d) {
        ...
        boolean isEvent = mDelegate.isHasEvent(calendar);
        boolean isLeaveDay = mDelegate.isLeaveDay(calendar);

        if (isSelected) {
            onDrawSelected(canvas, calendar, x, y, false);
        }
        if (mDelegate.isHasHighlight(calendar)) {
            onDrawHighlight(canvas, calendar, x, y);
        }
        //Disable draw leave day
//        if(isLeaveDay) {
//           onDrawLeaveDay(canvas, calendar, x, y);
//        }
        if (isEvent) {
            onDrawEvent(canvas, calendar, x, y);
        }
        onDrawText(canvas, calendar, x, y, hasScheme, isSelected, isEvent, isLeaveDay);
    }

    protected abstract void onDrawLeaveDay(Canvas canvas, Calendar calendar, int x, int y);
        ...
        if (mParentLayout != null) {
            if (calendar.isCurrentMonth()) {
                ...
                mParentLayout.updateSelectWeek(
                    CalendarUtil.getWeekFromDayInMonth(calendar, mDelegate.getWeekStart()));
            }
    ...
    protected abstract void onDrawEvent(Canvas canvas, Calendar calendar, int x, int y);

    protected abstract void onDrawHighlight(Canvas canvas, Calendar calendar, int x, int y);
     ...
     */
    protected abstract void onDrawText(Canvas canvas, Calendar calendar, int x, int y,
        boolean hasScheme, boolean isSelected, boolean isEvent, boolean isLeaveDay);
}
