...
public class CalendarView extends LinearLayout {
    ...
    public static final int BIIN_DOT_EVENT = 4;
    ...
    private final ViewPager.OnPageChangeListener onPageChangeListener = new ViewPager
        ...
        public void onPageSelected(int position) {
            ...
            calendar.add(Calendar.MONTH, position - FIRST_VISIBLE_PAGE);
    ...
    private void setAttributes(AttributeSet attrs) {
        TypedArray typedArray = getContext()
            .obtainStyledAttributes(attrs, R.styleable.CalendarView);
    ...
    private void initCalendarProperties(TypedArray typedArray) {
        ...
        int abbreviationsBarColor = typedArray
            .getColor(R.styleable.CalendarView_abbreviationsBarColor, 0);
        mCalendarProperties.setAbbreviationsBarColor(abbreviationsBarColor);

        int abbreviationsLabelsColor = typedArray
            .getColor(R.styleable.CalendarView_abbreviationsLabelsColor, 0);
        ...
        int anotherMonthsDaysLabelsColor = typedArray
            .getColor(R.styleable.CalendarView_anotherMonthsDaysLabelsColor, 0);
        ...
        int selectionLabelColor = typedArray
            .getColor(R.styleable.CalendarView_selectionLabelColor, 0);
        mCalendarProperties.setSelectionLabelColor(selectionLabelColor);

        int disabledDaysLabelsColor = typedArray
            .getColor(R.styleable.CalendarView_disabledDaysLabelsColor, 0);
        mCalendarProperties.setDisabledDaysLabelsColor(disabledDaysLabelsColor);

        int calendarType = typedArray.getInt(R.styleable.CalendarView_type, BIIN_DOT_EVENT);
        ...
        Drawable previousButtonSrc = typedArray
            .getDrawable(R.styleable.CalendarView_previousButtonSrc);
        mCalendarProperties.setPreviousButtonSrc(previousButtonSrc);

        Drawable forwardButtonSrc = typedArray
            .getDrawable(R.styleable.CalendarView_forwardButtonSrc);
    ...
    private void initAttributes() {
        ...
        AppearanceUtils
            .setHeaderLabelColor(getRootView(), mCalendarProperties.getHeaderLabelColor());

        AppearanceUtils.setAbbreviationsBarColor(getRootView(),
            mCalendarProperties.getAbbreviationsBarColor());

        AppearanceUtils.setAbbreviationsLabelsColor(getRootView(),
        ...
        AppearanceUtils
            .setPreviousButtonImage(getRootView(), mCalendarProperties.getPreviousButtonSrc());

        AppearanceUtils
            .setForwardButtonImage(getRootView(), mCalendarProperties.getForwardButtonSrc());
    ...
    private void setCalendarRowLayout() {
        ...
        } else if (mCalendarProperties.getCalendarType() == BIIN_DOT_EVENT) {
            mCalendarProperties.setItemLayoutResource(R.layout.calendar_view_day_with_dot);
        } else {
    ...
    private void callOnPageChangeListeners(int position) {
        if (position > mCurrentPage && mCalendarProperties
            .getOnForwardPageChangeListener() != null) {
            mCalendarProperties.getOnForwardPageChangeListener().onChange();
        }

        if (position < mCurrentPage && mCalendarProperties
            .getOnPreviousPageChangeListener() != null) {
    ...
    /**
     * @param onDayClickListener OnDayClickListener interface responsible for handle clicks on
     *                           calendar cells
     * @see OnDayClickListener
     */
    ...
    public void setDate(Calendar date) throws OutOfDateRangeException {
        if (mCalendarProperties.getMinimumDate() != null && date
            .before(mCalendarProperties.getMinimumDate())) {
            throw new OutOfDateRangeException("SET DATE EXCEEDS THE MINIMUM DATE");
        }

        if (mCalendarProperties.getMaximumDate() != null && date
            .after(mCalendarProperties.getMaximumDate())) {
    ...
    public Calendar getCurrentPageDate() {
        ...
        calendar.add(Calendar.MONTH, mViewPager.getCurrentItem() - FIRST_VISIBLE_PAGE);
        return calendar;
    }
    ...
    public void showMonthDate(Calendar month) {
        mViewPager.setCurrentItem(mViewPager.getCurrentItem()
            - DateUtils.getMonthsBetweenDates(month, getCurrentPageDate()), true);
    }
    ...
    public void setHasEventDays(List<Calendar> disabledDays) {
        mCalendarProperties.setHasEventDays(disabledDays);
    }

    public void setHighlightDays(List<Calendar> disabledDays) {
        mCalendarProperties.setmHighlightDays(disabledDays);
    }
}
