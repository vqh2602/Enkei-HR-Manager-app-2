...
class CalendarDayAdapter extends ArrayAdapter<Date> {
    ...
    public View getView(int position, View view, @NonNull ViewGroup parent) {
        if (view == null) {
            view = mLayoutInflater
                .inflate(mCalendarProperties.getItemLayoutResource(), parent, false);
        }

        TextView dayLabel = (TextView) view.findViewById(R.id.dayLabel);
        ImageView dayIcon = (ImageView) view.findViewById(R.id.dayIcon);
        View hlView = view.findViewById(R.id.hightlightView);
        ...
        if (dayIcon != null) {
            if (mCalendarProperties.getHasEventDays() != null && mCalendarProperties
                .getCalendarType() == CalendarView.BIIN_DOT_EVENT) {
            ...
            setLabelColors(dayLabel, day, isHasEvent(day));

            dayLabel.setText(String.valueOf(day.get(Calendar.DAY_OF_MONTH)));
            return view;
        }

        setLabelColors(dayLabel, day, false);

        dayLabel.setText(String.valueOf(day.get(Calendar.DAY_OF_MONTH)));
        return view;
    }

    private void setLabelColors(TextView dayLabel, Calendar day, boolean isHasEvent) {
        // Setting not current month day color
        if (!isCurrentMonthDay(day)) {
            DayColorsUtils
                .setDayColors(dayLabel, mCalendarProperties.getAnotherMonthsDaysLabelsColor(),
                    Typeface.NORMAL,
                    isHasEvent ? R.drawable.red_circle_selector : R.drawable
        ...
        if (!isActiveDay(day)) {
            DayColorsUtils.setDayColors(dayLabel, mCalendarProperties.getDisabledDaysLabelsColor(),
                Typeface.NORMAL,
                isHasEvent ? R.drawable.red_circle_selector : R.drawable.background_transparent);
            return;
        }

        // Setting current month day color
        DayColorsUtils
            .setCurrentMonthDayColors(day, mToday, dayLabel, mCalendarProperties, isHasEvent);
    }

    private boolean isSelectedDay(Calendar day) {
        return mCalendarProperties.getCalendarType() != CalendarView.CLASSIC && day
            .get(Calendar.MONTH) == mPageMonth
            && mCalendarPageAdapter.getSelectedDays().contains(new SelectedDay(day));
    }

    private boolean isCurrentMonthDay(Calendar day) {
        return day.get(Calendar.MONTH) == mPageMonth &&
            !((mCalendarProperties.getMinimumDate() != null && day
                .before(mCalendarProperties.getMinimumDate()))
                || (mCalendarProperties.getMaximumDate() != null && day
    ...
    private boolean isHasEvent(Calendar day) {
        return mCalendarProperties.getHasEventDays().contains(day);
    }

    private boolean isHighlight(Calendar day) {
        return mCalendarProperties.getHighlightDays().contains(day);
    }

    private void loadIcon(ImageView dayIcon, Calendar day) {
        if (mCalendarProperties.getEventDays() == null || (mCalendarProperties
            .getCalendarType() != CalendarView.CLASSIC && mCalendarProperties
            .getCalendarType() != CalendarView.ONE_DAY_PICKER)) {
            dayIcon.setVisibility(View.GONE);
            return;
        }
