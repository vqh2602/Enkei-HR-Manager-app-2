...
//                            smoothZoomTo(1.0f, getWidth() / 2.0f, getHeight() / 2.0f);
        ...
        ev.setLocation(zoomX + (x - 0.5f * getWidth()) / zoom,
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (pinching || ld > 30.0f) {
                    ...
                    smoothZoomTo(Math.max(1.0f, zoom * d / (d - dd)), zoomX - dxk / zoom,
                        zoomY - dyk / zoom);
                }
    ...
    protected void dispatchDraw(final Canvas canvas) {
        ...
        final boolean animating = Math.abs(zoom - smoothZoom) > 0.0000001f
            || Math.abs(zoomX - smoothZoomX) > 0.0000001f || Math
        ...
        if (showMinimap) {
            ...
            canvas.drawRect(dx - 0.5f * w / zoom, dy - 0.5f * h / zoom, dx + 0.5f * w / zoom,
                dy + 0.5f * h / zoom, p);
